...
layout(location = 10) uniform vec4 rpColorModulate;
layout(location = 11) uniform vec4 rpColorAdd;
layout(location = 12) uniform vec4 rpDiffuseColor;
layout(location = 13) uniform vec4 rpSpecularColor;
...
void main(void)
{
  vec4 color = texture2D(texture1, fragment.texcoord);
  result = color * 0.3;
}